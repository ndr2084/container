...
def get_args():
    ...
    parser.add_argument("--rl-endpoint", dest="rl_endpoint", type=str, default=None,
                            help="endpoint for RlSchedulerScore plugin")
    parser.add_argument("--rl-timeout-ms", dest="rl_timeout_ms", type=int, default=None,
                            help="timeout in ms for RlSchedulerScore plugin")
    ...
    for k, v in template.items():
        if k == "profiles":
            ...
            for item in template['profiles'][0]['plugins']['score']['enabled']:
                ...
                plugin_args = {'dimExtMethod': args.dim_ext_method, 'normMethod': args.norm_method}
                if item['name'] == "RlSchedulerScore":
                    plugin_args = {}
                    if args.rl_endpoint:
                        plugin_args['rlEndpoint'] = args.rl_endpoint
                    if args.rl_timeout_ms:
                        plugin_args['rlTimeoutMs'] = args.rl_timeout_ms
                pc.append({'name': item['name'], 'args': plugin_args})
